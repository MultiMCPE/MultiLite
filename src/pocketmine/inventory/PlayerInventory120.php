...
class PlayerInventory120 extends PlayerInventory {
	...
	public function getItem($index) {
		if ($index < 0) {
			switch ($index) {
				case self::CURSOR_INDEX:
					return $this->cursor == null ? Item::get(Item::AIR, 0, 0) : clone $this->cursor;
				case self::CRAFT_INDEX_0:
				case self::CRAFT_INDEX_1:
				case self::CRAFT_INDEX_2:
				case self::CRAFT_INDEX_3:
				...
				default:
					if ($index <= self::QUICK_CRAFT_INDEX_OFFSET) {
						$slot = self::QUICK_CRAFT_INDEX_OFFSET - $index;
						return !isset($this->quickCraftSlots[$slot]) || $this->quickCraftSlots[$slot] == null ? Item::get(Item::AIR, 0, 0) : clone $this->quickCraftSlots[$slot];
					}
					break;
			}
	...
	public function clear($slotIndex, $send = true) {
		if (isset($this->slots[$slotIndex])) {
			if ($this->isArmorSlot($slotIndex)) { //Armor change
				$ev = new EntityArmorChangeEvent($this->holder, $this->slots[$slotIndex], Item::get(Item::AIR, 0, 0), $slotIndex);
				...
				$ev = new EntityInventoryChangeEvent($this->holder, $this->slots[$slotIndex], Item::get(Item::AIR, 0, 0), $slotIndex);
				Server::getInstance()->getPluginManager()->callEvent($ev);
				if ($ev->isCancelled()) {
	...
	public function clearAll($send = true) {
		...
		for ($index = self::CRAFT_INDEX_0; $index >= self::CRAFT_INDEX_8; $index--) {
			$this->setItem($index, Item::get(Item::AIR, 0, 0));
		}
		$this->cursor = null;
	}

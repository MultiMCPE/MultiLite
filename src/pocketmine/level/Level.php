...
class Level implements ChunkManager, Metadatable
{
    ...
    public function broadcastLevelSoundEvent(Vector3 $pos, string $sound, int $pitch = 1, int $extraData = -1) {
    ...
    */
    ...
    public function requestChunk(int $x, int $z, Player $player){
        ...
        if (isset($this->chunkCache[$index][$player->getChunkProtocol()])) {
		...
		$pk->chunkX = $x;
		$pk->chunkZ = $z;
		$pk->data = $chunk->networkSerialize($player->getChunkProtocol());
		$pk->encode($player->getChunkProtocol());

		$batch = new BatchPacket();
		$batch->payload = zlib_encode(Binary::writeUnsignedVarInt(strlen($pk->buffer)) . $pk->buffer, ZLIB_ENCODING_DEFLATE, 7);
		$batch->encode($player->getChunkProtocol());
        
        $this->chunkCache[$index][$player->getChunkProtocol()] = $batch->getBuffer();
        $player->sendChunk($x, $z, $this->chunkCache[$index][$player->getChunkProtocol()]);
